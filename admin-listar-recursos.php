...
<head>
    ...
    <title>Listado Recursos Kairós</title>
    <link
...
<form method="GET" class="container mt-3 mb-2">
  <div class="input-group">
    ...
    <?php if (isset($_GET['buscar_id']) && $_GET['buscar_id'] !== ''): ?>
      <a href="admin-listar-recursos.php" class="btn btn-primary rounded-start ms-2">Ver todas</a>
    <?php endif; ?>
  </div>
</form>
...
<?php endif; ?>
  ...
  <div class="titulo-listar">Recursos registrados</div>
  <a href="admin-add-recursos.html" class="btn btn-primary">Crear Recurso</a>
</div>
  ...
  <div class="container-eventos">
      <table class="table mt-2">
          ...
          <tbody>
            ...
            if ($buscar_id !== '') {
            $sql = "SELECT id_recurso, nombre_recurso, descripcion_recurso,tipo_recurso, autor_recurso, imagen_recurso FROM recursos WHERE id_recurso = " . intval($buscar_id);
            } else {
            $sql = "SELECT id_recurso, nombre_recurso, descripcion_recurso,tipo_recurso, autor_recurso, imagen_recurso FROM recursos";
            }
            $result = mysqli_query($conn, $sql);
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    while($row = mysqli_fetch_assoc($result)) {
                        ...
                        echo "<td>" . $row['nombre_recurso'] . "</td>";
                        echo "<td>" . $row['descripcion_recurso'] . "</td>";
                        echo "<td>" . $row['tipo_recurso'] . "</td>";
                        echo "<td>" . $row['autor_recurso'] . "</td>";
                        echo "<td>"; 
              if (!empty($row['imagen_recurso'])) {
                  echo "<img src='./imagenes/" . $row['imagen_recurso'] . "' alt='Imagen del recurso' width='100'>";  
              } else {
              ...
              echo "<a href='admin-editar-recurso.php?id_recurso=" . $row["id_recurso"] . "' class='btn btn-primary'>Editar</a>"; 
              echo "<button class='btn btn-primary' onclick='eliminarRecurso(" . $row['id_recurso'] . ")'>Eliminar</button>"; 
              echo "</div>"; 
              echo "</td>";
              ...
              echo "</tr>";
                    ...
                    echo "<tr><td colspan='6'>No hay recursos registrados.</td></tr>";
                }
  ...
  function eliminarRecurso(id_recurso) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location.href = 'admin-delete-recurso.php?id_recurso=' + id_recurso;
      }
    });
  }
