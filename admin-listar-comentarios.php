...
<head>
    ...
    <title>Listado Comentarios Kairós</title>
    <link
...
<form method="GET" class="container mt-3 mb-2">
  <div class="input-group">
    <input type="text" name="buscar_id" class="form-control" placeholder="Buscar por ID de comentario" value="<?php echo isset($_GET['buscar_id']) ? htmlspecialchars($_GET['buscar_id']) : ''; ?>">
    <button type="submit" class="btn btn-primary rounded-end">Buscar</button>
    <?php if (isset($_GET['buscar_id']) && $_GET['buscar_id'] !== ''): ?>
      <a href="admin-listar-comentarios.php" class="btn btn-primary rounded-start ms-2">Ver todas</a>
    <?php endif; ?>
  </div>
</form>
...
<?php endif; ?>
  ...
  <div class="titulo-listar">Lista de comentarios</div>
  ...
  <div class="container-eventos">
      <table class="table mt-2">
          ...
          <tbody>
            ...
            if ($buscar_id !== '') {
            $sql = "SELECT id_opinion, opinion, fecha FROM opiniones WHERE id_opinion = " . intval($buscar_id);
            } else {
            $sql = "SELECT id_opinion, opinion, fecha FROM opiniones";
            }
            $result = mysqli_query($conn, $sql);
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    while($row = mysqli_fetch_assoc($result)) {
                        ...
                        echo "<button class='btn btn-primary' onclick='eliminarComentario(" . $row['id_opinion'] . ")'>Eliminar</button>"; 
                        echo "</div>"; 
                        echo "</td>";
                        echo "</tr>";
                    }
                } else {
                    echo "<tr><td colspan='6'>No hay opiniones registradas.</td></tr>";
                }
...
<script>
  function eliminarComentario(id_opinion) {
    Swal.fire({
      title: '¿Estás seguro?',
      ...
      if (result.isConfirmed) {
        window.location.href = 'admin-delete-comentario.php?id_opinion=' + id_opinion;
      }
    });
  }
